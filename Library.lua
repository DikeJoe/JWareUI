...
local function CreateWindow(opts)
	...
	local theme  = opts.Theme or Themes.JWare3
	...
	local function bgFrame(props, parent)
		return track(tracked.bgFrames, makeFrame(props, parent))
	end
	local function bg2Frame(props, parent)
		return track(tracked.bg2Frames, makeFrame(props, parent))
	end
	...
	local overlayGui = makeScreenGui("JWare UI Overlays",   guiParent)
	...
	local popupOverlay = makeFrame({ Name = "PopupOverlay", BackgroundTransparency = 1, Size = UDim2.new(1, 0, 1, 0), ZIndex = 200 }, mainGui)
 
	-- ── Keybind panel ─────────────────────────────────────────────────────────
	local keybindFrame = bgFrame({
		Name = "KeybindFrame", BackgroundColor3 = theme.bg,
	...
	local keybindContent = bg2Frame({
		Name = "Content", BackgroundColor3 = theme.bg2,
	...
	local watermarkFrame = bgFrame({
		Name = "WatermarkFrame", BackgroundColor3 = theme.bg,
	...
	function Window:SetWatermark(text, alignment)
		...
		makeLabel({
			Size = UDim2.new(1, 0, 1, 0), Text = text, TextSize = 13,
			TextColor3 = Color3.fromRGB(255, 255, 255), TextXAlignment = toTextAlign(alignment),
	...
	function Window:AddTab(tabOpts)
		...
		local tabButton = makeLabel({
			Name = tabOpts.Title, BackgroundColor3 = theme.main, BackgroundTransparency = 1,
			TextSize = 14, TextColor3 = COLOR_TEXT_DIM, FontFace = Font.new("rbxasset://fonts/families/Ubuntu.json"),
		...
		local tabIcon = make("ImageLabel", {
			BackgroundTransparency = 1, Image = tabOpts.Icon, ImageColor3 = COLOR_TEXT_DIM,
			Size = UDim2.new(0, 20, 0, 20), Position = UDim2.new(0, -10, 0.25, 0), BorderSizePixel = 0,
		}, tabButton)
 
		local container = bgFrame({
			Name = tabOpts.Title .. "_Container", BackgroundColor3 = theme.bg,
			Size = UDim2.new(1, -10, 0, H - 90), Position = UDim2.new(0, 5, 0, 50), Visible = false,
		}, contentFrame)
		...
		function Tab:AddSection(secOpts)
			...
			if not self[colKey] then
				local col = makeFrame({
					Name = colKey, BackgroundTransparency = 1,
			...
			local sectionFrame = bg2Frame({
				Name = secOpts.Title, BackgroundColor3 = theme.bg2, Size = UDim2.new(0, 218, 0, 40),
			}, self[colKey])
			...
			track(tracked.gradients, make("UIGradient", {
				Rotation     = 90,
			...
			makeLabel({
				Name = "Title", Size = UDim2.new(1, 0, 0, 15), Position = UDim2.new(0, 0, 0, 5),
				Text = secOpts.Title, TextSize = 14, TextColor3 = Color3.fromRGB(255, 255, 255),
				FontFace = Font.new("rbxasset://fonts/families/Ubuntu.json"), ZIndex = 3,
			}, sectionFrame)
 
			local elemHolder = makeFrame({
				Name = "ElementsHolder", BackgroundTransparency = 1,
				Position = UDim2.new(0, 0, 0, 30), Size = UDim2.new(0, 218, 0, 0),
			}, sectionFrame)
			...
			function Section:AddButton(cfg)
				...
				local btn = make("TextLabel", {
					Name = cfg.Title, BackgroundColor3 = COLOR_ELEM_BG, BorderSizePixel = 0,
					Size = UDim2.new(0, 208, 0, 20), Text = cfg.Title, TextSize = 14,
					TextColor3 = COLOR_TEXT_NORMAL, Font = Enum.Font.Gotham, ClipsDescendants = true,
				}, self.elemHolder)
			...
			function Section:AddToggle(cfg)
				cfg = cfg or {}
				applyDefaults({
					Title = "Toggle", Default = false, Callback = function() end,
					KeybindEnabled = false, KeyBind = "None", Mode = "Toggle",
					Sync = false, KeyCallback = function() end,
				}, cfg)
				...
				local titleLabel = makeLabel({
					Name = "Title", Size = UDim2.new(0, 150, 1, 0), Position = UDim2.new(0, 23, 0, 0),
					Text = cfg.Title, TextSize = 14, TextXAlignment = Enum.TextXAlignment.Left,
					TextColor3 = COLOR_TEXT_NORMAL, Font = Enum.Font.Gotham,
				}, row)
				...
				if cfg.KeybindEnabled then
					keyLabel = makeLabel({
						Size = UDim2.new(0, 30, 0, 15), Position = UDim2.new(0, 177, 0, 0),
						Text = activeKey, TextSize = 12,
						TextColor3 = toggled and theme.main or COLOR_TEXT_DIM, Font = Enum.Font.GothamBold,
					}, row)
				...
				local function doToggle()
					toggled = not toggled; refreshCheck(true); cfg.Callback(toggled)
				...
				return {
					frame = row, check = check, keyLabel = keyLabel,
			...
			function Section:AddSlider(cfg)
				...
				local trackFrame  = makeFrame({ Name = "Track", BackgroundColor3 = COLOR_ELEM_BG, Size = UDim2.new(1,0,0,15), Position = UDim2.new(0,0,0,20) }, sliderFrame)
				addOutlineStroke(trackFrame, 1)
				local fill = track(tracked.fills, makeFrame({ BackgroundColor3 = theme.main }, trackFrame))
				...
				local function applyMouseX(mouseX)
					local rel = math.clamp(mouseX - trackFrame.AbsolutePosition.X, 0, trackFrame.AbsoluteSize.X)
					local pct = trackFrame.AbsoluteSize.X > 0 and (rel / trackFrame.AbsoluteSize.X) or 0
				...
				trackFrame.InputBegan:Connect(startDrag); fill.InputBegan:Connect(startDrag)
				...
				return {
					...
					SetValue = function(v)
						...
						currentValue = round(v); fill.Size = UDim2.new(pct, 0, 1, 0)
						valueLabel.Text = tostring(math.floor(currentValue)) .. cfg.Suffix
						cfg.Callback(currentValue)
					end,
				}
			...
			function Section:AddDropdown(cfg)
				...
				local listScroll = track(tracked.scrollbars, make("ScrollingFrame", {
					...
					ScrollBarImageColor3 = theme.main, ZIndex = 200,
				}, listFrame))
				...
				UIS.InputBegan:Connect(function(input)
					...
					if not (mp.X >= lp.X and mp.X <= lp.X+ls.X and mp.Y >= lp.Y and mp.Y <= lp.Y+ls.Y)
					and not (mp.X >= dp.X and mp.X <= dp.X+ds.X and mp.Y >= dp.Y and mp.Y <= dp.Y+ds.Y) then
						setExpanded(false)
					end
				end)
				...
				local function buildOptions(options)
					...
					for _, name in ipairs(options) do
						local wrapper = make("TextButton", {
							Name = name, BackgroundTransparency = 1, AutoButtonColor = false,
							Size = UDim2.new(1, 0, 0, 20), ZIndex = 200, Text = "",
						}, listScroll)
						local optLabel = makeLabel({
							Name = "Label", BackgroundColor3 = COLOR_ELEM_BG, BackgroundTransparency = 0,
							Size = UDim2.new(1, 0, 1, 0), Text = name, TextSize = 14, TextColor3 = COLOR_TEXT_NORMAL,
							TextXAlignment = Enum.TextXAlignment.Left, Font = Enum.Font.Gotham, ZIndex = 200,
						}, wrapper)
			...
			function Section:AddColorPicker(cfg)
				...
				local svBox      = makeFrame({ BackgroundColor3 = Color3.fromRGB(255,0,0), Size = UDim2.new(0,SVW,0,SVH), Position = UDim2.new(0,PAD,0,PAD), ZIndex = 201 }, panel)
				...
				make("UIGradient", { Rotation = 90, Color = ColorSequence.new{
					ColorSequenceKeypoint.new(0.000, Color3.fromRGB(255,0,0)),   ColorSequenceKeypoint.new(0.166, Color3.fromRGB(255,255,0)),
					ColorSequenceKeypoint.new(0.333, Color3.fromRGB(0,255,0)),   ColorSequenceKeypoint.new(0.500, Color3.fromRGB(0,255,255)),
				...
				local function loadColor(c)
					hue, sat, val_ = Color3.toHSV(c); refreshPicker(false)
				end
				...
				local function applyHue(mp)
					hue = math.clamp((mp.Y - hueBar.AbsolutePosition.Y) / hueBar.AbsoluteSize.Y, 0, 1)
					refreshPicker(false)
				end
				...
				UIS.InputBegan:Connect(function(input)
					...
					if not (mp.X>=pp.X and mp.X<=pp.X+ps.X and mp.Y>=pp.Y and mp.Y<=pp.Y+ps.Y)
					and not (mp.X>=sp.X and mp.X<=sp.X+ss.X and mp.Y>=sp.Y and mp.Y<=sp.Y+ss.Y) then
						panel.Visible = false; if activePopup == panel then activePopup = nil end
					end
				end)
				...
				return {
					frame = row, swatch = swatch, panel = panel,
			...
			function Section:AddTextInput(cfg)
				cfg = cfg or {}
				applyDefaults({
					Title = "Text Input", Default = "", Placeholder = "Enter text...",
					ClearOnFocus = true, Numeric = false, MaxLength = nil,
					Callback = function() end, OnFocus = function() end, OnUnfocus = function() end,
				}, cfg)
				...
				local textBox    = make("TextBox", {
					...
					Text = cfg.Default, PlaceholderText = cfg.Placeholder, PlaceholderColor3 = COLOR_TEXT_DIM,
					TextSize = 13, TextColor3 = COLOR_TEXT_NORMAL, TextXAlignment = Enum.TextXAlignment.Left,
					Font = Enum.Font.Gotham, ClearTextOnFocus = cfg.ClearOnFocus, ClipsDescendants = true,
				}, boxFrame)
				...
				return {
					frame = inputFrame, textBox = textBox,
					GetValue = function() return currentValue end,
					SetValue = function(v)
						v = tostring(v)
